...
class GenerateSprite(Resource):
    def post(self):
        ...
        try:
            ...
            gdprep.spencode(im, "./tmp/{0}".format(fname), (width, height),"PALETTE256A", True)  # por enquanto nao vamos permitir mudar o a paleta e compress
            with open("./tmp/{0}.h".format(fname), "rb") as file:
                data = file.read()  # nao sei se eh a melhor maneira; talvez se retornarmos o arquivo direto 
